...
class CrawlerService:
    ...
    async def crawl_for_task(self, task_id: str) -> TaskCrawlSummary:
        ...
        # ── 1. Discover all websites attached to this task ────────────────────
        stmt_websites = (
            select(Website)
        ...
        # ── 2. Update task stage to CRAWLING & Immediate Commit ───────────────
        task.status = "RUNNING"
        ...
        self.session.add(
            ScrapingLog(
                ...
                message=f"Starting website crawler for task {task.task_id} ({total_websites} websites queued).",
            )
        )
        ...
        try:
            ...
            logger.debug("Realtime publish failed for CRAWLING stage start: %s", pe)
        ...
        # ── 3. Crawl Websites with Failure Isolation & Concurrency ──────────
        concurrency_limit = getattr(settings, "CRAWLER_CONCURRENCY", 3)
        site_sem = asyncio.Semaphore(concurrency_limit)
        site_timeout = getattr(settings, "WEBSITE_CRAWL_TIMEOUT_SECONDS", 30.0)

        async def _crawl_single_website(site: Website) -> tuple[Website, WebsiteCrawlResult]:
            async with site_sem:
                logger.info("Task %s: Crawling website %s (%s)", task.task_id, site.url, site.domain)
                try:
                    res = await asyncio.wait_for(
                        self.crawler.crawl(
                            website_id=site.id,
                            root_url=site.url,
                            max_pages=min(task.max_pages_per_site, 10),
                    ...
                    logger.warning("Website %s crawl timed out after %.1fs", site.url, site_timeout)
                    return site, WebsiteCrawlResult(
                        website_id=site.id,
                        url=site.url,
                        domain=site.domain,
                        status="FAILED",
                        error_reason=f"Crawl timed out after {site_timeout:.0f}s",
                        duration_seconds=site_timeout,
                    )
                except Exception as exc:
                    logger.error("Error crawling website %s: %s", site.url, exc)
                    return site, WebsiteCrawlResult(
                        website_id=site.id,
                        url=site.url,
                        domain=site.domain,
        ...
        if websites:
            futures = [_crawl_single_website(w) for w in websites]
            for future in asyncio.as_completed(futures):
                website, crawl_res = await future
