...
class ExtractionService:
    ...
    async def extract_for_task(
        ...
        # ── 1. Retrieve all SourcePages for this task ─────────────────────────
        ...
        # ── 2. Update task stage to EXTRACTING & Immediate Commit ─────────────
        task.status = "RUNNING"
        ...
        self.session.add(
            ScrapingLog(
                ...
                message=f"Beginning data extraction for task {task.task_id} ({total_pages} source pages).",
            )
        )
        ...
        try:
            ...
            logger.debug("Realtime publish failed for EXTRACTING stage start: %s", pe)

        # Helper to fetch HTML using persistent client with connection pooling
        ...
        async def get_html(page_url: str) -> str:
            ...
            try:
                ...
                if html_cache is not None and text:
                    html_cache[page_url] = text
                return text
        ...
        try:
            for idx, page in enumerate(source_pages, 1):
                ...
                html_text = await get_html(page.url)
                if not html_text:
                    continue
