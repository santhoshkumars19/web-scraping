"""
app/services/export/export_query.py

Reusable export query builder for LeadScout.
Extracts unpaginated matching Lead records with full relationship eager-loading
to guarantee zero N+1 queries.
"""
...
class ExportQueryBuilder:
    """Builds and executes unpaginated queries for lead export generation."""
    ...
    async def get_export_leads(
        ...
        *,
        ...
        """Execute query returning full Lead records without pagination limits."""
        base_query = (
            select(Lead)
        ...
        if lead_id is not None:
            target_uuid = lead_id if isinstance(lead_id, uuid.UUID) else uuid.UUID(str(lead_id))
            conditions.append(Lead.id == target_uuid)

        # ── Selected IDs scoping ──────────────────────────────────────────────
        if lead_ids:
